...
// in agentRunner.js when unset. CLAUDE_BIN only applies when AGENT_KIND is
// still claude-code - otherwise a leftover CLAUDE_BIN=claude in an existing
// .env (from before AGENT_KIND was switched to codex) would silently spawn
// the wrong CLI with the wrong adapter's arguments.
const CLAUDE_BIN = process.env.AGENT_BIN || (AGENT_KIND === 'claude-code' ? process.env.CLAUDE_BIN : '') || '';
